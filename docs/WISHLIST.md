...
| S-026 | **Confirmed 2026-09-23: `spawn` measured 1.6 s on the work machine**, worse as the day goes on, sometimes near-free — the signature of a reputation cache being evicted. The shell half of C-031: start the warm process (lazily on first Ctrl+R, or in the background at shell init so even the first is paid for), talk to it over a named pipe, and **fall back to a cold spawn whenever it does not answer in time** — a wedged helper must never be able to hang Ctrl+R (principle 7) | M2 | planned |
| S-029 | `scripts/measure-spawn.ps1`: tell apart "this binary is being inspected" from "every CreateProcess is hooked", by timing a signed Microsoft binary, hit, and a freshly-named copy of hit. The two have different fixes and only one of them needs code, so this runs before C-031 is worth building | M1 | done |
