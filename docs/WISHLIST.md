...
| S-026 | **Confirmed 2026-09-23: `spawn` measured 1.6 s on the work machine**, worse as the day goes on, sometimes near-free. S-029 found the cause: a first-ever launch of an unknown file costs ~3.5 s and ~58 ms thereafter, so this is a reputation cache being evicted and refilled — and our own release cadence refills it, because every new version is a new unknown file. The shell half of C-031: start the warm process (lazily on first Ctrl+R, or in the background at shell init so even the first is paid for), talk to it over a named pipe, and **fall back to a cold spawn whenever it does not answer in time** — a wedged helper must never be able to hang Ctrl+R (principle 7) | M2 | planned |
