...
| S-031 | **A resident finder keeps running the binary it was started from**, so after an install the old code goes on answering Ctrl+R in every shell that already has one — which on a machine that ships often means testing a change against the version it replaced. Hit during the T-008 work: `alt+d` appeared to be missing until the shell was restarted. **Done:** `install.ps1` stops running servers unless `-KeepServers` is passed, which is safe because every failure on that path falls back to spawning. **Still open, and nicer:** a version handshake — `hit init pwsh` already stamps the binary's version into the module, so the ping reply could carry the server's version and the shell could abandon a mismatched one by itself. That costs nothing on the hot path (the ping already happens), needs no install script to remember anything, and fixes the case the script cannot reach: a shell that was already open on another desktop | M2 | idea |
